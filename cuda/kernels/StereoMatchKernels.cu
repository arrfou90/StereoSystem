...
__global__ void kernelStereoMatchL2R(unsigned char* dev_leftImg,
	...
	if (doCalc) {
		...
		int bestSAD = 65532;
		int bestDisparity = 0;
		unsigned char* pX1L = dev_leftImg + (idx - halfKS - halfKS * width)*bytesPerPixel;// Zeiger auf Anfang des Kernel
		unsigned char* pX1R = dev_rightImg + (idx - halfKS - halfKS * width)*bytesPerPixel;
